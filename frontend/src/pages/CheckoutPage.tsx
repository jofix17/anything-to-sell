  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        await cartService.confirmPayment(orderId, paymentIntent.id);
        onSuccess();
      }
    } catch (error) {
...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      try {
        setIsLoading(true);
        const userAddresses = await cartService.getAddresses();
        ...
        if (defaultAddress) {
          ...
          setSelectedShippingAddress(userAddresses[0].id);
          setSelectedBillingAddress(userAddresses[0].id);
        }
      } catch (error) {
  ...
  const handleAddressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      const address = await cartService.createAddress(newAddress);
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const { clientSecret } = await cartService.createPaymentIntent(order.id);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center">
          <div className={`h-10 w-10 rounded-full flex items-center justify-center ${step === 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
            1
          </div>
          <div className="flex-1 h-1 mx-4 bg-gray-200">
            <div className={`h-full ${step >= 2 ? 'bg-indigo-600' : 'bg-gray-200'}`} style={{ width: step >= 2 ? '100%' : '0%' }}></div>
          </div>
          <div className={`h-10 w-10 rounded-full flex items-center justify-center ${step === 2 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
            2
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {step === 1 && (
            <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
              <h2 className="text-xl font-bold mb-6">Shipping & Billing Information</h2>
              ...
              <div className="mb-8">
                ...
                {showAddressForm ? (
                  <form onSubmit={handleAddressSubmit} className="space-y-4 border rounded-md p-4 bg-gray-50">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {addresses.map(address => (
                            <option key={address.id} value={address.id}>
                              {address.fullName} - {address.addressLine1}, {address.city}, {address.state} {address.postalCode}, {address.country}
                        ...
                        {selectedShippingAddress && (
                          <div className="mt-3 p-3 border rounded-md bg-gray-50">
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.fullName}<br />
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.addressLine1}<br />
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.addressLine2 && (
                              <>{addresses.find(addr => addr.id === selectedShippingAddress)?.addressLine2}<br /></>
                            )}
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.city}, {addresses.find(addr => addr.id === selectedShippingAddress)?.state} {addresses.find(addr => addr.id === selectedShippingAddress)?.postalCode}<br />
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.country}<br />
                            {addresses.find(addr => addr.id === selectedShippingAddress)?.phoneNumber}
                          </div>
                        )}
                      </div>
                    ) : (
                ...
                {!sameAsShipping && (
                  <div>
                    ...
                    {selectedBillingAddress && (
                      <div className="mt-3 p-3 border rounded-md bg-gray-50">
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.fullName}<br />
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.addressLine1}<br />
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.addressLine2 && (
                          <>{addresses.find(addr => addr.id === selectedBillingAddress)?.addressLine2}<br /></>
                        )}
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.city}, {addresses.find(addr => addr.id === selectedBillingAddress)?.state} {addresses.find(addr => addr.id === selectedBillingAddress)?.postalCode}<br />
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.country}<br />
                        {addresses.find(addr => addr.id === selectedBillingAddress)?.phoneNumber}
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={createOrder}
                  disabled={isLoading || !selectedShippingAddress || (sameAsShipping ? false : !selectedBillingAddress)}
